...
- Nothing inside the container is scheduled. If you use automation
  Wait steps, flows, or appointment reminders, point an external scheduler at
  `GET /api/automations/cron`, `GET /api/flows/cron`, and
  `GET /api/appointments/cron` on this deployment, sending the shared secret
  in the `x-cron-secret` header (`AUTOMATION_CRON_SECRET`, see `.env.local.example`).
  These return 503 until that variable is set.
